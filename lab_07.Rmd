...
# I noticed the county name. the county fips code and population number doesn't appear in the population table. This can cause problems down the road because it makes it difficult to make in-depth analysis on the data. 
```
...
geo_pills_population_state_2009%>%
  ...
  geom_sf(lwd = 0) +
  theme_map() +
  labs(fill='Population',title="West Virginia had the highest pills per person total in 2020", subtitle = "2009 population, U.S. Census", caption = "Source: U.S. Census ACS") +
...
geo_pills_population_state %>%
  ...
  geom_sf(lwd = 0) +
  theme_map() +
  labs(fill='Pills per Person',title="South Dakota's pills per person total increased each year from 2006-2014.", subtitle = "", caption = "Source: ARCOS database") +
  theme(legend.position="bottom") +
  scale_fill_viridis_c(option = "magma",labels = comma)
...
pills_maryland_population <- pills_population %>%
  filter(year==2006, 2012, buyer_state=="MD") %>%
  group_by(statefips, buyer_state, buyer_county) %>%
  summarise(total_pills=sum(dosage_unit, na.rm = TRUE),
            total_population=sum(population, na.rm=TRUE)) %>%
  mutate(pills_per_person=total_pills/total_population)
...
geo_pills_population_state <- state_geodata_shifted %>%
 inner_join(pills_maryland_population, by=c("GEOID" = "statefips"))

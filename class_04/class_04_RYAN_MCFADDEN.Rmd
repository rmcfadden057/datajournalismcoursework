...
* How many rows does the data have? How many columns?   The data has 42 columns and 298,908 rows. 
* Each column is stored in one of two formats: "chr", or strings of characters, generally used for text; and "dbl", a number format on which mathematical operations can be performed.  Do you see any examples of a column stored as a "dbl" that doesn't really need to be, because we would never do math on it? Are there any columns stored as character that you think should be stored in another format entirely? I think the dbl_columns MME Conversion Factor, CALC_BASE_WT_IN_GM, and dos_str. I think Reporter Zip should be store as a character. 
* Based on the information here, how are missing values represented in this data set? They are represented as "null" 
...
**Task**: Which single pharmacy **location** had the most pills sent to it? Newcare Home Health Services
...
# A federal jury convicted the owners of Newcare for illegally selling pills over the Internet, conspiracy to launder money and filing false tax returns. The state of Maryland shut down the company. NewCare purchased 9.9 million dollars worth of hydrocone.
...
#McKesson Corporation, which is located in Maryland, sent 49,834,830 pills. 
...
baltimore %>%
  ...
  group_by(reporter_name, reporter_state, buyer_name, buyer_city, buyer_address1) %>%
  summarise(total_pills= sum(dosage_unit)) %>% 
  arrange(desc(total_pills))

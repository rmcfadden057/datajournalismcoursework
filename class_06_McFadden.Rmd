...
```{r}
#The Maryland graph and the Baltimore graph show some similarity as the numbers between 2010 and 2014 seemed to be steady. 
...
```{r}
  ...
  labs(x="Year", y="Total pills", title="In Maryland, there was an increase in pills shipped from 2006 to 2011", subtitle = "Total pills shipped to Maryland by year", caption = "Source: DEA ARCOS database, via Washington Post") +
...
```{r}
state_population <- state_population(key = key) %>%
  clean_names()
```

```{r}
#Part1
by_state_pills_2012 <- arcos_county_pills_per_year %>%
  filter(year=="2012") %>%
...
by_state_population_2012 <- county_population(key = key) %>%
  clean_names() %>%
  filter(year=="2012") %>%
...
by_state_population_2012 <- by_state_pills_2012 %>%
  inner_join(by_state_population_2012, by=("buyer_state")) %>%
  select(buyer_state, dosage_unit, year, population)
...
ggplot(state_pills_population_2012) +
  ...
  geom_text_repel(aes(population, dosage_unit, label=buyer_county),
                  subset(west_virginia_2012, population > 5000))

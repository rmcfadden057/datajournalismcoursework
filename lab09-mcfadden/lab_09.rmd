...
pills_population_income %>%
 ...
 group_by(quintile) %>%
 summarise(median_household_income = median(median_household_income),
            average_pills_per_person = median(average_pills_per_person))  
...
ggplot(pills_population_income) +
  ...
  scale_x_continuous(labels = comma)  +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  labs(x="Average Annual Pills Per Person", y="Household Income", title="", caption = "Source: DEA ARCOS database, via Washington Post")
...
```
One thing to take away from this analysis is that the lowest median income house total has the most pills per person. In Sumter County, the median household income is less than 24,000 but the average pills per year is over 300,000 while the average pills per person is 22.45. In Dallas County, the average pills per year is over 430,000 while the median household income is around 26,000. 

...
pills_west_virginia_population <- pills_population %>%
  filter(buyer_state=="WV") %>%
  group_by(countyfips, buyer_state, buyer_county, year) %>%
...
geo_pills_population_county_west_virginia <- county_geodata_shifted %>%
 inner_join(pills_west_virginia_population, by=c("GEOID" = "countyfips"))
```
...
```{r}
#In Marion County, there were never really huge spikes or drops in the pills per person or total pills. The numbers kind of stayed steady over the years. 
